...
## 🤖 Natural Language Order Creation with LangGraph + MongoDB + Hybrid Search

This guide explains the **AI Order Assistant** - a natural language interface for creating orders using **LangGraph** state machines, **Grove API** (OpenAI GPT-5.4), **MongoDB Atlas**, and **MongoDB Hybrid Search** 🆕.

---

## 🎯 What It Does

The AI Order Assistant allows users to create orders using natural language instead of structured JSON, with **intelligent inventory validation** and **semantic product matching** 🆕.
...
├─────────────────────────────────────────┤
│  Node 3: Product Matcher (MongoDB)     │ ← 🆕 Hybrid Search (Vector + Text)
│         with Hybrid Search 🆕           │    - Semantic matching with Voyage AI
...
- **MongoDB Hybrid Search** 🆕: Vector Search + Text Search for semantic product matching
- **Voyage AI** 🆕: voyage-4-large model for generating embeddings (1024 dimensions)
- **Spring Boot**: REST API framework with @Transactional support
- **Apache Commons Text**: Fuzzy string matching (fallback)
...
### Node 3: Product Matcher 🆕 (with Hybrid Search)
...
#### Strategy 1: MongoDB Hybrid Search (Recommended) 🆕

**When Available:** Products have embeddings generated by Voyage AI

**How It Works:**
1. **Generate Query Embedding**: Convert user's description to 1024-dimensional vector using Voyage AI
2. **Vector Search**: Find semantically similar products using MongoDB `$vectorSearch`
3. **Ranking**: Sort by cosine similarity score
4. **Return**: Top matching product with high confidence (0.95)

**Benefits:**
- ✅ **Semantic Understanding**: "running shoes" matches "Athletic Sneakers"
- ✅ **Context Aware**: "gift for developer" matches "MongoDB Guide" book
- ✅ **Typo Tolerant**: "laptpo" still finds "Laptop Pro 15"
- ✅ **Multi-language Ready**: Works across languages (if embeddings support it)

**Example:**
```
User: "I want something comfortable for running"
Query Embedding: [0.234, -0.567, 0.891, ..., 0.123] (1024 dims)
Vector Search: Finds "Athletic Sneakers" (cosine similarity: 0.92)
Result: "Athletic Sneakers" (score: 0.95)
```

**Technology:**
- Voyage AI (voyage-4-large model)
- MongoDB Atlas Vector Search
- Cosine similarity matching

#### Strategy 2: Fuzzy Matching (Fallback)

**When Used:** Products don't have embeddings yet
...
**Automatic Detection:**
The system automatically detects if hybrid search is available and uses the best strategy:
```
...
💡 Run embedding generation to enable hybrid search
```

**Setup:** See [MONGODB_HYBRID_SEARCH_SETUP.md](MONGODB_HYBRID_SEARCH_SETUP.md) for instructions.
